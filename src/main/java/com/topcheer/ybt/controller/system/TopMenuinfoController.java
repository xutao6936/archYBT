...
public class TopMenuinfoController {
	private static Logger log = LoggerFactory
			.getLogger(TopMenuinfoController.class);
	@Autowired
	...
	@ResponseBody
	public PageInfo<TopMenuinfo> getTopMenuinfoList(String rows, String page,
			TopMenuinfo topMenuinfo) {
		Map<String, Object> map = Maps.newHashMap();
		map.put("topMenuinfo", topMenuinfo);
		map.put("pageSize", rows);
		map.put("pageNo", page);
		PageInfo<TopMenuinfo> searchTopMenuinfo = topMenuinfoService
				.searchTopMenuinfo(map);
	...
	@ResponseBody
	public Map<String, Object> insert(@Valid TopMenuinfo topMenuinfo,
	...
	@ResponseBody
	public Map<String, Object> update(@Valid TopMenuinfo topMenuinfo,
			BindingResult result, HttpServletRequest request) {
		if (result.hasErrors()) {
			return ResultHelper.analyzeError(result);
		}
	...
	@ResponseBody
	public Map<String, Object> oper(@Valid TopMenuinfo topMenuinfo,
			BindingResult result, HttpServletRequest request) {
	...
	public String getMenu(HttpServletRequest request) {
		...
		for (TopMenuinfo topMenuinfo : menulist) {
			if ("YBT".equals(topMenuinfo.getFaMenuCode())) {// 一级菜单
				List<TopMenuinfo> lists = iteratorMenus(menulist,
						topMenuinfo.getMenuCode());
				topMenuinfo.setMenulist(lists);// 将子菜单放入到当前的一级菜单中
				list.add(topMenuinfo);
			}
		}
		Gson gson = new Gson();
		String jsonString = gson.toJson(list).replaceAll("menuCode", "id")
				.replaceAll("menuName", "text").replaceAll("menuUrl", "url")
				.replaceAll("icon", "icon").replaceAll("menulist", "menus");
	...
	// 递归菜单
	private static List<TopMenuinfo> iteratorMenus(List<TopMenuinfo> menuList,
			String pid) {
		...
		for (TopMenuinfo menuVo : menuList) {
			...
			if (StringUtils.isNotBlank(parentid) && !"YBT".equals(parentid)) {
				if (parentid.equals(pid)) {
					List<TopMenuinfo> iterateMenu = iteratorMenus(menuList,
							menuid);
